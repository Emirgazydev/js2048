...
window.navigator.events = undefined;
...
KeyboardInputManager.prototype.listen = function () {
    ...
    // Реагировать на клавиши направления
    document.addEventListener("keydown", function (event) {
        let modifiers = event.altKey || event.ctrlKey || event.metaKey ||
            event.shiftKey;
        let mapped = map[event.which];

        // Игнорировать событие, если оно происходит в текстовом поле
    ...
    gameContainer.addEventListener(this.eventTouchstart, function (event) {
        if ((!window.navigator.msPointerEnabled && event.touches.length > 1) ||
            ...
            return; // Игнорировать, если касание более чем 1 пальцем или касание ввода
        }
    ...
    gameContainer.addEventListener(this.eventTouchend, function (event) {
        if ((!window.navigator.msPointerEnabled && event.touches.length > 0) ||
            ...
            self.targetIsInput(event)) {
            return; //Игнорировать, если все еще касаетесь одним или несколькими пальцами или вводите
        }
...
function ga(send, event, game, restart) {
    
}

// Продолжить игру (как перезапустить, так и продолжить игру)
...
HTMLActuator.prototype.scoreTweetButton = function () {
    ...
    const text = "I scored " + this.score + " points at 2048, a game where you " +
